...
static jobject MakeJObject(JNIEnv *env, const apriltag_detection_t *detect) {
  ...
  wpi::java::JLocal<jstring> fam{
      env, wpi::java::MakeJString(env, detect->family->name)};
  ...
  wpi::java::JLocal<jdoubleArray> carr{
      env, wpi::java::MakeJDoubleArray(
               env, {reinterpret_cast<const jdouble *>(corners), 4 * 2})};
...
static jobjectArray MakeJObject(JNIEnv *env, const zarray_t *detections) {
  jobjectArray jarr =
      env->NewObjectArray(zarray_size(detections), detectionCls, nullptr);
  if (!jarr) {
    return nullptr;
  }
...
Java_org_photonvision_jni_GpuDetectorJNI_createGpuDetector(JNIEnv *jenv,
                                                           jobject jobj,
                                                           jint width,
                                                           jint height) {
  ...
  detector.gpu_detector_ = new frc971::apriltag::GpuDetector(
      width, height, detector.apriltag_detector_, detector.camera_matrix,
...
JNIEXPORT void JNICALL Java_org_photonvision_jni_GpuDetectorJNI_setparams(
    JNIEnv *jenv, jobject jobj, jlong handle, jdouble fx, jdouble cx,
    jdouble fy, jdouble cy, jdouble k1, jdouble k2, jdouble p1, jdouble p2,
    jdouble k3) {
  ...
  std::cout << "fx " << fx << " cx " << cx << " fy " << fy << " cy " << cy
            << std::endl;
  std::cout << "k1 " << k1 << " k2 " << k2 << " p1 " << p1 << " p2 " << p2
            << " k3 " << k3 << std::endl;
  detectors[handle].camera_matrix =
      frc971::apriltag::CameraMatrix{fx, cx, fy, cy};
  detectors[handle].dist_coeffs =
      frc971::apriltag::DistCoeffs{k1, k2, p1, p2, k3};
  delete (detectors[handle].gpu_detector_);
  detectors[handle].gpu_detector_ = new frc971::apriltag::GpuDetector(
      cols, rows, detectors[handle].apriltag_detector_,
      detectors[handle].camera_matrix, detectors[handle].dist_coeffs);
}

JNIEXPORT void JNICALL
Java_org_photonvision_jni_GpuDetectorJNI_destroyGpuDetector(JNIEnv *jenv,
                                                            jobject jobj,
                                                            jlong handle) {
...
Java_org_photonvision_jni_GpuDetectorJNI_processimage(JNIEnv *jenv,
                                                      jobject jobj,
                                                      jlong handle, jlong p) {
  if (!p) {
  ...
  if (!detectors[handle].gpu_detector_) {
    ...
    delete (detectors[handle].gpu_detector_);
    detectors[handle].gpu_detector_ = new frc971::apriltag::GpuDetector(
        img.cols, img.rows, detectors[handle].apriltag_detector_,
        detectors[handle].camera_matrix, detectors[handle].dist_coeffs);
  }
  ...
  if (zarray_size(detections))
    for (int i = 0; i < zarray_size(detections); ++i) {
      // std::cout << "971 library has " << zarray_size(detections) << "
      // detections" << std::endl;
      apriltag_detection_t *gpu_detection;
      zarray_get(detections, i, &gpu_detection);
      std::cout << "tag: " << gpu_detection->id << " at " << gpu_detection->c[0]
                << "," << gpu_detection->c[1] << std::endl;
    }
